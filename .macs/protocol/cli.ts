...
switch (command) {
  ...
  case 'status': {
    ...
    if (m.review_required > 0) console.log(`        ${m.review_required} awaiting review`)
    if (m.pending_human > 0) console.log(`        ${m.pending_human} escalated to human`)
    ...
    if (activeTasks.length > 0) {
      ...
      for (const t of activeTasks) {
        ...
        if (t.last_checkpoint_at) console.log(`   last checkpoint: ${timeAgo(t.last_checkpoint_at)}`)
        if (t.parent_task) console.log(`   subtask of: ${t.parent_task}`)
      }
    }
    ...
    // Show review queue
    const reviewTasks = Object.values(state.tasks).filter(t =>
      t.status === 'review_required' || t.status === 'under_review'
    )
    if (reviewTasks.length > 0) {
      ...
      for (const t of reviewTasks) {
        ...
        if (t.reviewer) console.log(`   reviewer: ${t.reviewer}`)
        if (t.review_requested_at) console.log(`   requested: ${timeAgo(t.review_requested_at)}`)
      }
    }
    ...
    if (escalatedTasks.length > 0) {
      ...
      for (const t of escalatedTasks) {
        ...
        if (t.escalation_reason) console.log(`   reason: ${t.escalation_reason}`)
        if (t.escalated_to) console.log(`   escalated to: ${t.escalated_to}`)
        if (t.escalated_at) console.log(`   since: ${timeAgo(t.escalated_at)}`)
      }
    }
  ...
  case 'block': {
    ...
    if (!taskId) { console.error('Usage: macs block <task-id> --reason "..." --next "..."'); process.exit(1) }
  ...
  case 'cancel': {
    ...
    const agentId = getArg('--agent') || 'cli'
    const reason = getArg('--reason') || 'unspecified'
    if (!taskId) { console.error('Usage: macs cancel <task-id> --reason "..." --next "..."'); process.exit(1) }
  ...
  case 'checkpoint': {
    const taskId = args[1]
    const agentId = getArg('--agent') || 'cli'
    if (!taskId) { console.error('Usage: macs checkpoint <task-id> --agent <id> --note "✓ done → next"'); process.exit(1) }
  ...
  case 'decompose': {
    const taskId = args[1]
    const agentId = getArg('--agent') || 'cli'
    if (!taskId) { console.error('Usage: macs decompose <task-id> --into "subtask1,subtask2"'); process.exit(1) }

    const into = getArg('--into')
    if (!into) { console.error('--into "subtask1,subtask2" is required'); process.exit(1) }

    const subtaskTitles = into.split(',').map(s => s.trim()).filter(Boolean)
    if (subtaskTitles.length < 2) { console.error('At least 2 subtasks required'); process.exit(1) }
  ...
  case 'swarm': {
    ...
    let agentDefs: Array<{ name: string; capabilities: string[] }>
    ...
    if (!isNaN(agentCount) && agentCount > 0) {
      ...
      agentDefs = agentSpec.split('|').map(s => {
        ...
        if (colonIdx === -1) return { name: s.trim(), capabilities: [] as string[] }
        return {
    ...
    ;(async () => {
      ...
      if (isSimulate) {
        ...
        while (true) {
          ...
          const batch: Array<{ agent: string; taskId: string }> = []
        ...
        if (m.blocked > 0) console.log(`   ⚠️  ${m.blocked} blocked (need manual intervention)`)
        if (m.pending > 0) console.log(`   ⏳ ${m.pending} pending (unsatisfied dependencies?)`)
  ...
  case 'review': {
    const taskId = args[1]
    const agentId = getArg('--agent') || 'cli'
    const result = getArg('--result') as 'approved' | 'rejected' | undefined
    if (!taskId) { console.error('Usage: macs review <task-id> --agent <id> --result approve|reject [--note "..."]'); process.exit(1) }
  ...
  case 'escalate': {
    ...
    if (!taskId) { console.error('Usage: macs escalate <task-id> --reason "..." [--to human-id] [--timeout 60]'); process.exit(1) }
    if (!reason) { console.error('❌ --reason is required for escalation'); process.exit(1) }
    ...
    if (escalate_to) console.log(`   escalated to: ${escalate_to}`)
    if (timeout_ms) console.log(`   auto-resume after: ${timeoutMin} min`)
    break
  }
