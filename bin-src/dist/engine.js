...
export class MACSEngine {
            ...
            switch (event.type) {
                ...
                case 'task_blocked': {
                    ...
                    if (task) {
                        ...
                        if (event.data.handoff_note)
                            task.handoff_note = event.data.handoff_note;
                    }
                    break;
                }
                case 'task_unblocked': {
                ...
                case 'task_updated': {
                    const task = tasks[event.id];
                    if (task) {
                        task[event.data.field] = event.data.to;
                    }
                    break;
                }
                ...
                case 'task_review_requested': {
                    ...
                    if (task) {
                        ...
                        if (event.data.suggested_reviewer)
                            task.reviewer = event.data.suggested_reviewer;
                    }
                    break;
                }
                case 'task_reviewed': {
    ...
    findTasks(filter) {
        ...
        if (filter.capable_agent) {
            ...
            tasks = tasks.filter(t => {
                if (!t.requires_capabilities || t.requires_capabilities.length === 0)
                    return true;
    ...
    decomposeTask(agentId, parentTaskId, subtaskTitles, rationale) {
        const state = this.getState();
        const parentTask = state.tasks[parentTaskId];
        if (!parentTask)
            throw new Error(`Task ${parentTaskId} not found`);
    ...
    completeTask(agentId, taskId, data) {
        this.appendTaskEvent({
            type: 'task_completed',
            id: taskId,
            ts: new Date().toISOString(),
    ...
    cancelTask(agentId, taskId, data) {
        this.appendTaskEvent({
            type: 'task_cancelled',
            id: taskId,
            ts: new Date().toISOString(),
    ...
    unblockTask(agentId, taskId, data) {
        this.appendTaskEvent({
            type: 'task_unblocked',
            id: taskId,
            ts: new Date().toISOString(),
        ...
        for (const task of Object.values(state.tasks)) {
            ...
            if (silentMs >= threshold) {
                results.push({
                    task,
                    silentMs,
                    level: (silentMs > threshold * 2 ? 'confirmed' : 'suspected'),
                });
            }
        }
    ...
    submitReview(reviewerId, taskId, data) {
        this.appendTaskEvent({
            type: 'task_reviewed',
            id: taskId,
            ts: new Date().toISOString(),
    ...
    // Escalation (3.11)
    // ----------------------------------------------------------
    escalateTask(agentId, taskId, data) {
        this.appendTaskEvent({
            type: 'task_escalated',
            id: taskId,
            ts: new Date().toISOString(),
    ...
    // Lock Operations
    // ----------------------------------------------------------
    acquireLock(agentId, file, reason, eta_ms) {
        const state = this.getState();
    ...
    sendMessage(msg) {
        ...
        const existing = existsSync(inboxDir)
            ? readJsonl(join(inboxDir, 'messages.jsonl')).length
            : 0;
    ...
    analyzeImpact(file) {
        ...
        for (const task of Object.values(state.tasks)) {
            ...
            if (isAffected) {
                ...
                if (task.assignee)
                    affectedAgents.add(task.assignee);
            }
        }
        return {
